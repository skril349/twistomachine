...
from screens.graphics_twist import create_twist_plot_screen as create_twist_plot_screen  # Importa la función desde graphics.py
...
def start_motor_and_graphic(fixed_entries, input_fields, root, window_geometry):
    ...
    create_twist_plot_screen(root, window_geometry)
...
def print_inputs(fixed_entries, input_fields,root, window_geometry):
    ...
    # Calcular el valor de "Trigger" com la mitad de "Time pause"
    time_pause = int(input_data.get("Time pause:", 0))
    input_data["Trigger"] = str(time_pause / 2)
...
def create_screen(root, window_geometry):
    ...
    btn_add = tk.Button(buttons_frame, text="Add position", command=lambda: add_input_field(buttons_frame, input_fields))
    btn_add.pack(side='top', fill='x', padx=5, pady=5)

    # Botó per eliminar l'últim input dinàmic afegit
    btn_delete = tk.Button(buttons_frame, text="Delete position", command=lambda: delete_input_field(input_fields, buttons_frame))
    ...
    path = 'assets/twist_graph.png'  # Actualitza el camí segons la ubicació de les teves imatges
